...
Finally, the marginal superiority of FBTS over DE on the DROZY dataset (COR $-0.89$ vs.\ $-0.94$, though both are negative) suggests that covariance-based features may be slightly more robust to the sparse, subjective labeling inherent in clinical drowsiness assessments. While our DROZY results confirm that per-epoch regression is unsuitable for test-level KSS annotations, they point toward a promising future direction: **FBTS features paired with test-level or session-level aggregation strategies** for clinically realistic labeling scenarios. This is not a failure of the FBTS method itself, but rather a mismatch between the algorithm's epoch-level prediction paradigm and DROZY's sparse annotation design. Future work will explore temporal aggregation techniques that align FBTS features with test-level labels.
...
Several limitations should be acknowledged. First, while our LOSO evaluation reveals FBTS's cross-subject advantage, explicit Riemannian domain adaptation methods~\cite{zanini2018transfer} could further close the gap to within-subject performance. Second, we did not evaluate deep temporal models (LSTM, Transformer) on FBTS features, which could leverage the rich tangent space representation for sequence modeling~\cite{zhang2016continuous,li2023transformer}. Self-supervised pre-training~\cite{wang2024selfsupervised} may further improve data efficiency. That said, we emphasize that traditional feature engineering with linear/SVR models remains irreplaceable for data-scarce scenarios and resource-constrained edge devices---a strength confirmed by our data efficiency results showing 94\% performance retention with only 50\% training data. Third, FBTS with 62-channel EEG (SEED dataset) was computationally challenging; efficient SPD matrix approximations or channel selection strategies are needed for high-density montages. Fourth, the negative results on DROZY (Section~\ref{tab:drozy}) highlight the fundamental incompatibility between per-epoch regression and sparse test-level annotations---a challenge that extends to any similarly labeled dataset.

% =================================================================
\section{Conclusion}

This paper presents the first systematic adaptation and validation of Riemannian filter bank tangent space (FBTS) features---originally designed for motor imagery classification---for continuous EEG-based vigilance regression. On the SEED-VIG dataset with 23 subjects, our key findings are: (1) single-modality FBTS matches DE (COR 0.520 vs.\ 0.521; $t(22)=-0.02$, $p=0.984$); (2) FBTS+EOG fusion significantly outperforms DE+EOG (COR 0.618 vs.\ 0.586; $t(22)=2.13$, $p=0.044$, $d=0.45$); (3) subject-wise LOSO reveals that FBTS+EOG with 6 temporal channels achieves mean COR=0.835---the strongest cross-subject result, a 60\% improvement over within-subject DE; (4) a lightweight 4-channel forehead variant retains 99\% of full-channel performance (COR 0.613); and (5) cross-dataset validation on SEED emotion recognition achieves 86.1\% accuracy. Regarding SEED FBTS, our preliminary 10-channel evaluation (55.8\% accuracy) is constrained by the tenfold channel reduction (10 vs.\ 62), which dramatically reduces covariance matrix rank and information content. Full 62-channel FBTS evaluation remains a priority for future work, pending computational optimizations. We note that while FBTS, like all per-epoch regression approaches, produces negative results on DROZY due to that dataset's sparse test-level annotations, this limitation is methodological---not specific to FBTS---and affects all continuous-frame prediction algorithms; FBTS features paired with test-level aggregation strategies represent a promising future direction. FBTS thus constitutes a general-purpose, channel-agnostic EEG feature extraction framework with particular strength in multimodal fusion and resource-constrained scenarios, with clear translational potential for clinical applications including OSA monitoring, narcolepsy diagnosis, and automotive fatigue warning systems.
