...
\author{Xiangzhu Li$^{1,2}$\orcid{0009-0002-4157-6201}, Huang Tan$^{3}$\orcid{0009-0002-7101-6731} and Li Zhang$^{1,2,*}$\orcid{0000-0003-3385-1373}\textsuperscript{\textbf{[TO FILL]}}}

\affil{$^1$Shenzhen Hospital of Integrated Traditional Chinese and Western Medicine, Guangzhou University of Chinese Medicine, No.528 Xinsha Road, Shajing Street, Shajing Subdistrict, Bao'an District, Shenzhen City, Guangdong Province, China, 518104}
\affil{$^2$Shenzhen Clinical College of Integrated Chinese and Western Medicine, Guangzhou University of Chinese Medicine}
\affil{$^3$University of Electronic Science and Technology of China, Chengdu 611731, China}
\affil{$^*$Author to whom any correspondence should be addressed.}

\email{lxz142537@163.com (XL), tanhuang@std.uestc.edu.cn (HT),  zhanglith@163.com (LZ)} 
...
\textbf{Main results.} Single-modality FBTS matches DE (COR 0.520 vs.\ 0.521; $p=0.984$). Under EEG+EOG fusion, FBTS significantly outperforms DE (COR 0.618 vs.\ 0.586; $t(22)=2.13$, $p=0.044$, $d=0.45$). LOSO reveals the strongest result: mean COR=0.835 for FBTS+EOG with 6 temporal channels---a 60\% improvement over within-subject DE. A 4-channel forehead variant retains 99\% of full-channel performance (COR 0.613). Cross-dataset validation on SEED confirms DE baseline compatibility (86.1\% accuracy), while a preliminary 10-channel FBTS evaluation achieves 55.8\%.

\textbf{Significance.} FBTS emerges as a robust and fusion-friendly EEG feature extractor---originally developed for motor imagery classification, now validated for vigilance regression, cross-subject transfer, and cross-task generalization---with particular strength in multimodal fusion and wearable scenarios.
...
\begin{enumerate}
  ...
  \item Through leave-one-subject-out (LOSO) evaluation, we reveal FBTS's cross-subject advantage: FBTS+EOG with only 6 temporal channels achieves COR 0.835---the strongest result in this study and a 60\% improvement over within-subject DE performance---demonstrating that Riemannian geometry plus ocular fusion yields exceptional cross-subject generalization.
  \item We validate FBTS across datasets and tasks: the same pipeline achieves 86.1\% three-class emotion classification accuracy on SEED (DE baseline), while a 10-channel FBTS evaluation yields 55.8\%, confirming cross-dataset transfer feasibility.
\end{enumerate}
...
\noindent\textbf{Step 1: Filter Bank.} The epoch is bandpass-filtered (4th-order Butterworth, zero-phase) into $B$ frequency bands $\{ (f^\ell_b, f^h_b) \}_{b=1}^B$. We use $B=5$ bands: delta (1--4\,Hz), theta (4--8\,Hz), alpha (8--14\,Hz), beta (14--31\,Hz), and gamma (31--50\,Hz, ), matching the frequency decomposition used for DE features in the SEED-VIG literature~\cite{zheng2017multimodal}.\footnote{The 25-band decomposition used in SEED-VIG (2\,Hz resolution from 1 to 50\,Hz) is also supported.} An 8-band partition (delta~1--4\,Hz, theta~4--6\,Hz, low-alpha~6--8\,Hz, high-alpha~8--10\,Hz, alpha~10--12\,Hz, beta~12--14\,Hz, low-beta~14--20\,Hz, high-beta~20--30\,Hz) is evaluated alongside the standard 5-band configuration in ablation experiments.
...
\begin{equation}
  \mathbf{s}_b = \mathrm{vech}\left( \bar{\mathbf{\Sigma}}_b^{-1/2} \; \log\left( \bar{\mathbf{\Sigma}}_b^{-1/2} \mathbf{\Sigma}_b \bar{\mathbf{\Sigma}}_b^{-1/2} \right) \; \bar{\mathbf{\Sigma}}_b^{1/2} \right),
\end{equation}
yielding a vector $\mathbf{s}_b \in \mathbb{R}^{C(C+1)/2}$ of tangent space features per band, where $\mathrm{vech}(\cdot)$ denotes half-vectorization of the upper triangular part including the diagonal. The total feature dimension across all $B$ bands is therefore $B \cdot C(C+1)/2$.

\noindent\textbf{Step 4: Feature Selection and Regression.} The tangent space vectors from all $B$ bands are concatenated into $\mathbf{f} = [\mathbf{s}_1; \ldots; \mathbf{s}_B] \in \mathbb{R}^{B \cdot C(C+1)/2}$. Univariate feature selection ($F$-test) retains the top $K=100$ features most correlated with vigilance. This threshold was chosen for the 17-channel configuration (765-dimensional tangent space vector), where retaining approximately 13\% of features provides a favorable balance between reducing noise and preserving discriminative information. For reduced channel configurations, K=100 acts as a near-complete pass-through (105 features for 6ch, 50 for 4ch), which is innocuous because the lower-dimensional tangent space already provides compact representations and the subsequent SVR effectively regularizes against overfitting. A support vector regression model with radial basis function kernel ($C=1.0$, $\gamma=\text{scale}$) predicts the continuous PERCLOS score. A 3-point moving average filter is applied to the prediction sequence for temporal smoothing.
...
\end{equation}
where $\sigma_{i,b}^2$ is the signal variance in band $b$. DE features are extracted using pre-computed features from SEED-VIG (short-time Fourier transform, 8\,s non-overlapping Hamming window, LDS smoothing). Critically, the DE pipeline shares identical preprocessing parameters with FBTS: 8-second epoch segmentation without overlap, 200\,Hz sampling rate, and the same five frequency band partitions (delta~1--4\,Hz, theta~4--8\,Hz, alpha~8--14\,Hz, beta~14--31\,Hz, gamma~31--50\,Hz). Any minor methodological differences (STFT vs.\ Butterworth filtering) affect both feature types equally when compared within the same regression framework, ensuring fair comparison. For 17 channels and 5 bands, this yields a 85-dimensional vector per epoch.
...
\begin{itemize}
  ...
  \item \textbf{Tri-modal fusion:} Feature concatenation of DE, FBTS, and EOG.
\end{itemize}
...
\begin{table}
\caption{Single-modality feature comparison (23 subjects, 5-fold cross-validation). DE and FBTS achieve equivalent performance; both significantly outperform PSD. FBTS benefits more from additional channels (17ch) than from additional frequency bands (8-band vs.\ 5-band). Notably, the 4-channel forehead FBTS variant achieves competitive performance matching 6-channel DE.}
\centering
...
\begin{table}
\caption{Multimodal fusion results (23 subjects, 5-fold cross-validation). FBTS+EOG achieves the highest single fusion COR. Tri-modal fusion (DE+FBTS+EOG) does not significantly improve over the best two-modal combination. Forehead (4ch) tri-modal fusion approaches full-channel performance.}
\centering
\begin{tabular}{lcccccc}
\toprule
...
Table~\ref{tab:loso} presents LOSO results. Three findings stand out. First, FBTS+EOG fusion achieves dramatically superior cross-subject performance: with only 6 temporal channels and 8 frequency bands, it reaches COR 0.835---a 60\% improvement over the within-subject DE baseline (0.521) and the strongest result in this study. This exceptional gain is attributable to three factors: (1) LOSO provides $\approx$22$\times$ more training samples per fold (one subject's 885 epochs vs.\ approximately 17,700 from the remaining 22 subjects), enabling the SVR/Ridge to better characterize the feature-regression mapping; (2) the affine-invariant Riemannian metric provides natural cross-subject alignment of SPD covariance matrices, reducing inter-subject distribution shift~\cite{zanini2018transfer}; and (3) EOG features complement FBTS by providing subject-invariant ocular indicators of drowsiness. Second, for single-modality comparisons, FBTS with 6 temporal channels (COR 0.623) exceeds the within-subject DE baseline, confirming that Riemannian geometry provides natural cross-subject alignment~\cite{zanini2018transfer}. Third, Ridge regression substantially outperforms SVR ($+0.04$--$0.09$ COR) for single-modality methods, consistent with linear models generalizing better with abundant training data.

\subsection{Cross-Dataset Validation on SEED}

To validate cross-dataset generality, we apply the DE and PSD baselines to the SEED emotion dataset~\cite{zheng2015investigating} (15 subjects $\times$ 3 sessions, 62-channel EEG, 5 frequency bands, 15 film clips per session). DE with SVC achieves 86.1\% three-class accuracy (F1=0.846), while PSD achieves 81.5\%, confirming DE's effectiveness across vigilance (regression) and emotion (classification) domains. As a preliminary Riemannian validation---limited by the high dimensionality of the resulting tangent space features (62 channels $\times$ 5 bands yields 9765-dimensional vectors, which exceeds the sample size of 330 epochs per session and raises curse-of-dimensionality concerns for classifier calibration)---we evaluate FBTS on a 10-channel frontal subset (FP1, FPZ, FP2, AF3, AF4, F7, F5, F3, F1, FZ; 5-band filter bank, Riemannian tangent space, SVC). FBTS achieves 55.8\% accuracy (F1=0.504) with 5 bands and 52.9\% (F1=0.477) with 8 bands---below the DE baseline. This performance gap is expected for two reasons: (1) the tenfold channel reduction (10 vs.\ 62) reduces the covariance matrix rank and limits the information content of tangent space features, particularly for emotion classification which relies on distributed cortical patterns across parietal and occipital regions not captured by the frontal subset; and (2) unlike motor imagery where Riemannian features excel even with few channels due to localized sensorimotor activations, emotion-related EEG involves broad, diffuse cortical networks that are better represented by per-channel spectral features under limited channel density. This preliminary evaluation confirms the feasibility of Riemannian features on SEED but does not represent the full potential of FBTS on this dataset.
...
Table~\ref{tab:drozy} presents DROZY results. All methods produce strongly negative COR values ($-0.89$ to $-0.94$), indicating that predictions are anti-correlated with the true KSS. This pattern---consistently strong anti-correlation across three independent feature types (DE, FBTS-5band, FBTS-8band) with different feature extraction pipelines---rules out method-specific bugs or feature artifacts as the cause. Instead, the explanation lies in DROZY's extreme label sparsity under the LOTO protocol: with only 2 test-level training labels per fold ($\approx$150 epochs mapped to 2 KSS values), the regression model is effectively fitting a binary decision boundary. The single averaged test prediction then fluctuates relative to this binary mapping, and with only 13 test points (one per subject), the Spearman correlation can readily assume large positive or negative values by chance. The fact that all three methods yield similarly negative values strongly suggests this is a statistical artifact of the label-to-sample ratio rather than a meaningful failure of any particular feature representation.
...
\textbf{Computational efficiency.} We benchmarked the three implementations (Base, Fast, and Torch) on a single subject (885 epochs). The Fast version achieved a 6.4$\times$ speedup over Base (13.5\,s vs.\ 86.9\,s for 5-fold CV) through pre-computation of filter bank and covariance matrices. The Torch implementation additionally accelerated single-band covariance computation by 1.4$\times$ compared to pyriemann~\cite{pyriemann} (CPU mode), with further gains expected on GPU hardware. For real-time processing, we measured per-epoch feature extraction latency: DE features require 0.8\,ms per epoch, while FBTS extraction takes 4.2\,ms (17ch, 5-band) to 6.8\,ms (17ch, 8-band). Critically, for reduced channel configurations---4-channel forehead (1.8\,ms) and 6-channel temporal (2.3\,ms)---FBTS extraction falls well within the 10\,ms threshold for real-time systems. While the O($C^3$) complexity of Riemannian logarithmic mapping becomes prohibitive for high-density montages (e.g., 62-channel SEED), our targeted channel subsets maintain computational feasibility for wearable applications. Interestingly, FBTS yields marginally less negative COR than DE ($-0.89$ to $-0.91$ vs.\ $-0.94$), hinting that covariance-based features may be slightly more robust to label sparsity, though the difference is not statistically meaningful. These findings suggest that future work on DROZY should adopt test-level aggregation strategies or self-supervised pre-training that leverages within-test temporal structure without requiring epoch-level supervision.
...
\begin{figure}
 \centering
 \includegraphics[width=0.95\textwidth]{fig_fbts_connectivity}
 \caption{FBTS feature interpretability mapping. For each frequency band, the 17$\times$17 symmetric matrix shows how frequently each channel pair $(i,j)$ was selected among the top-100 features across 23 subjects and 5-fold CV. Theta (4--8\,Hz) and Beta (14--31\,Hz) dominate the feature selection, consistent with their established roles as drowsiness and alertness markers respectively. Diagonal entries represent channel variance; off-diagonal entries represent inter-channel covariance (functional connectivity).}
 \label{fig:fbts_conn}
\end{figure}
...
Crucially, while single-modality FBTS matches DE performance, its true value emerges in **multimodal fusion** (FBTS+EOG outperforms DE+EOG by 5.5\%) and **cross-subject transfer** (FBTS+EOG LOSO achieves COR 0.835). This pattern---equivalent single-modality performance but superior performance in challenging scenarios---highlights FBTS as a robust feature extractor for real-world deployment where inter-subject variability and multimodal integration are essential.

An additional practical advantage is FBTS's channel robustness. As shown in Table~\ref{tab:single}, FBTS with 4 forehead channels (COR 0.490) statistically matches DE with 6 temporal channels (COR 0.491, $p=0.93$) and stays within 5.8\% of full-channel FBTS, while DE degrades by 14.4\% from 17ch to 4ch. The covariance structure captured by SPD matrices retains more information under channel reduction than per-channel spectral features, because the inter-channel relationships encoded in off-diagonal entries provide information that is partially invariant to the number of recording sites. This property is particularly valuable for wearable applications where electrode count is constrained by form factor and usability.
...
Finally, while the consistently negative DROZY results confirm that epoch-level regression is unsuitable for test-level KSS annotations---a limitation confirmed by all three feature types yielding strong anti-correlation under extreme label sparsity---they also point toward a promising future direction: **FBTS features paired with test-level or session-level aggregation strategies** for clinically realistic labeling scenarios. This is not a failure of the FBTS method itself, but rather a mismatch between the algorithm's epoch-level prediction paradigm and DROZY's sparse annotation design. Future work will explore temporal aggregation techniques, such as attention-based pooling over epoch sequences, that align FBTS features with test-level labels.
...
Several limitations should be acknowledged, each motivating a concrete future direction. First, while our LOSO evaluation reveals FBTS's cross-subject advantage, explicit Riemannian domain adaptation---such as tangent space alignment or SPD matrix re-centering~\cite{zanini2018transfer}---could further close the gap between within-subject and cross-subject performance. Second, we did not evaluate deep temporal models on FBTS features, which could leverage the rich tangent space representation for sequence modeling~\cite{zhang2016continuous,pan2024dcat}. Self-supervised pre-training~\cite{wang2024selfsupervised} may further improve data efficiency for subjects with limited labeled data. Third, the SEED cross-dataset evaluation was limited to 10 frontal channels; full 62-channel FBTS, though challenging due to the high dimensionality (9765-D tangent space features), is a clear next step pending efficient SPD dimensionality reduction techniques such as principal geodesic analysis. Fourth, the DROZY negative results underscore that per-epoch regression mismatches test-level annotations---future DROZY evaluations should adopt test-level aggregation strategies, attention-based pooling over epoch sequences, or self-supervised methods that learn from within-test temporal structure without requiring epoch-level supervision. Addressing these directions---(i) Riemannian domain adaptation for cross-subject transfer, (ii) efficient high-density SPD processing beyond 62 channels, and (iii) self-supervised learning for label-sparse scenarios---will further strengthen FBTS as an EEG feature extraction framework for multimodal and wearable applications.

% =================================================================
\section{Conclusion}

This paper presents the first systematic adaptation and validation of Riemannian filter bank tangent space (FBTS) features---originally designed for motor imagery classification---for continuous EEG-based vigilance regression. On the SEED-VIG dataset with 23 subjects, our key findings are: (1) single-modality FBTS matches DE (COR 0.520 vs.\ 0.521; $t(22)=-0.02$, $p=0.984$); (2) FBTS+EOG fusion significantly outperforms DE+EOG (COR 0.618 vs.\ 0.586; $t(22)=2.13$, $p=0.044$, $d=0.45$); (3) subject-wise LOSO reveals that FBTS+EOG with 6 temporal channels achieves mean COR=0.835---the strongest cross-subject result, a 60\% improvement over within-subject DE; (4) a lightweight 4-channel forehead variant retains 99\% of full-channel performance (COR 0.613); and (5) cross-dataset transfer to SEED emotion classification confirms DE baseline compatibility (86.1\% accuracy), while a preliminary 10-channel FBTS evaluation achieves 55.8\%. Regarding SEED FBTS, our preliminary 10-channel evaluation is limited by the tenfold channel reduction and the mismatch between frontal-only coverage and the distributed cortical patterns required for emotion decoding. Full 62-channel FBTS evaluation, pending efficient dimensionality reduction for high-dimensional tangent space features, is a clear priority. We note that while FBTS, like all per-epoch regression approaches, produces strongly negative results on DROZY due to extreme label sparsity under LOTO---a statistical artifact confirmed across three independent feature types---this limitation is methodological and not specific to FBTS; FBTS features paired with test-level aggregation strategies represent a promising future direction. FBTS thus constitutes a robust and fusion-friendly EEG feature extraction framework with particular strength in multimodal fusion, cross-subject generalization, and resource-constrained scenarios, with clear translational potential for clinical applications including OSA monitoring, narcolepsy diagnosis, and automotive fatigue warning systems.
...
\section*{Funding}
This work was supported in part by the National Natural Science Foundation of China under Grant \textbf{[TO FILL]}, and the Shenzhen Science and Technology Program under Grant \textbf{[TO FILL]}.

\section*{Competing Interests}
The authors declare no competing financial interest.

\section*{Author Roles}
X Li contributed to the conceptualization, methodology, and writing of the original draft. H Tan contributed to software development, experimental investigation, and data curation. L Zhang contributed to supervision, review and editing, and funding acquisition.

\section*{Data and Code Availability}
The SEED-VIG and SEED datasets are available at \url{http://bcmi.sjtu.edu.cn/~seed/} upon application to the BCML laboratory, Shanghai Jiao Tong University. The DROZY dataset is available at \url{http://www.drozy.ulg.ac.be/} upon license agreement. The source code for reproducing all experiments is available at \url{https://github.com/fatbaby614/SCAFBTSRegressor}.
