#!/usr/bin/env python3
"""Regressor ablation: compare 4 regressors on FBTS features."""
import sys, os, json, time, numpy as np
from datetime import datetime

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from config import SEED_VIG_ROOT, RESULTS_DIR, REGRESSORS, DEFAULT_QUICK_N, DEFAULT_N_JOBS
...
def evaluate_one_regressor(n_subjects, regressor_name):
    ...
    for subj in subjects:
        ...
        clf = SCAFBTSRegressorFast(freq_bands='5band', estimator='oas',
            metric='riemann', regressor=regressor_name, n_features=100, fs=sr_s,
...
def main():
    ...
    print(f"Regressor Ablation: {n_subj} subjects x {len(REGRESSORS)} regressors")
    results = []
    for reg in REGRESSORS:
        print(f"  [{reg}] ...", end=' ', flush=True)
        t0 = time.time()
        try:
            r = evaluate_one_regressor(n_subj, reg)
            r['regressor'] = reg
    ...
    print(f"\n{'Regressor':<12s} {'COR':>8s} {'CORstd':>8s}")
    for r in results:
        print(f"{r['regressor']:<12s} {r['cor_mean']:>8.4f} {r['cor_std']:>8.4f}")
    print(f"\nBest: {results[0]['regressor']} ({results[0]['cor_mean']:.4f})")
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    with open(os.path.join(RESULTS_DIR, f'results_ablation_regressor_{ts}.json'), 'w') as f:
        json.dump(results, f, indent=2)
    print(f"Saved: results_ablation_regressor_{ts}.json")
